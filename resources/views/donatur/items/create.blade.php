    ...
    <div class="max-w-2xl mx-auto py-10 px-6">
        ...
        <div class="bg-white p-10 rounded-[40px] shadow-sm border border-[#E9EFE3]">
            ...
            <form action="{{ isset($item) ? route('donatur.items.update', $item) : route('donatur.items.store') }}" 
                ...
                {{-- Nama Barang --}}
                <div>
                    <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Nama Barang *</label>
                ...
                {{-- Tipe Listing --}}
                <div>
                    <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-3 ml-4">Tipe Listing</label>
                    <div class="flex gap-4 ml-2">
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="radio" name="tipe" value="donasi" x-model="tipe" class="text-[#43643C] focus:ring-[#43643C]">
                            <span class="text-sm font-bold text-[#43643C] group-hover:text-[#2C4027]">🎁 Donasi</span>
                        </label>
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="radio" name="tipe" value="jual" x-model="tipe" class="text-[#43643C] focus:ring-[#43643C]">
                            <span class="text-sm font-bold text-[#43643C] group-hover:text-[#2C4027]">🏷️ Jual Murah</span>
                        </label>
                    </div>
                </div>
                ...
                <div x-show="tipe === 'jual'" x-transition x-cloak>
                    <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Harga Satuan (Rp)</label>
                    <div class="relative">
                    ...
                    <div x-show="jumlah > 1 && harga > 0" class="mt-4 p-4 bg-[#F2F5EB] rounded-[20px] border border-[#E9EFE3] flex justify-between items-center transition-all">
                        <span class="text-xs font-black text-[#7A9375] uppercase tracking-widest">Total Harga (<span x-text="jumlah"></span> Barang)</span>
                        <span class="text-lg font-black text-[#43643C]" x-text="'Rp ' + (harga * jumlah).toLocaleString('id-ID')"></span>
                    </div>
                </div>

                {{-- Foto --}}
                <div>
                    <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Foto Barang *</label>
                    @if(isset($item) && $item->foto)
                        <img src="{{ Storage::url($item->foto) }}" class="h-32 rounded-[20px] mb-3 object-cover border-2 border-[#F2F5EB]">
                    @endif
                ...
                {{-- Kategori --}}
                <div>
                    <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Kategori *</label>
                ...
                <div x-show="kategori === 'lainnya'" x-transition x-cloak>
                    <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Nama Kategori *</label>
                    <input type="text" name="kategori_lainnya" value="{{ old('kategori_lainnya', $item->kategori_lainnya ?? '') }}"
                        class="w-full border-[#E9EFE3] bg-[#F9FBF7] rounded-full px-6 py-3 focus:border-[#43643C] focus:ring-[#43643C] text-[#2C4027]"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Jumlah *</label>
                    ...
                    <div>
                        <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Satuan *</label>
                        <input type="text" name="satuan" value="{{ old('satuan', $item->satuan ?? '') }}"
                            class="w-full border-[#E9EFE3] bg-[#F9FBF7] rounded-full px-6 py-3 focus:border-[#43643C] focus:ring-[#43643C]" placeholder="kg/pcs/dus">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Kota Domisili *</label>
                        <input type="text" name="kota" value="{{ old('kota', $item->kota ?? '') }}"
                            class="w-full border-[#E9EFE3] bg-[#F9FBF7] rounded-full px-6 py-3 focus:border-[#43643C] focus:ring-[#43643C]" placeholder="Contoh: Jakarta Selatan">
                    </div>
                    <div>
                        <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Tanggal Kedaluwarsa (Jika Ada)</label>
                        <input type="date" name="expiry_date" value="{{ old('expiry_date', isset($item) && $item->expiry_date ? $item->expiry_date->format('Y-m-d') : '') }}"
                            class="w-full border-[#E9EFE3] bg-[#F9FBF7] rounded-full px-6 py-3 focus:border-[#43643C] focus:ring-[#43643C]">
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Alamat Lengkap Pengambilan *</label>
                ...
                {{-- Deskripsi --}}
                <div>
                    <label class="block text-xs font-black text-[#7A9375] uppercase tracking-widest mb-2 ml-4">Deskripsi Singkat</label>
